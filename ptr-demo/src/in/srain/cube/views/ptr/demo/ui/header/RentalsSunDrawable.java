...
public class RentalsSunDrawable extends Drawable implements Animatable {
    ...
    private void drawSky(Canvas canvas) {
        ...
        float offsetY = // Offset canvas moving, goes lower when goes down
        y + 50 + mSkyTopOffset - // Offset sky scaling, lower than 0, will go greater when goes down
        mSkyHeight * (skyScale - 1.0f) / 2 + // Give it a little move top -> bottom  // will go greater when goes down
    ...
    private void drawTown(Canvas canvas) {
        ...
        float offsetY = y + +townTopOffset - // Offset town scaling
        mTownHeight * (townScale - 1.0f) / 2 + // Give it a little move
        townMoveOffset;
    ...
    private void drawSun(Canvas canvas) {
        ...
        float offsetY = mSunTopOffset + // Move the sun up
        (mTotalDragDistance / 2) * (1.0f - dragPercent);
        float scalePercentDelta = dragPercent - SCALE_START_PERCENT;
        if (scalePercentDelta > 0) {
            float scalePercent = scalePercentDelta / (1.0f - SCALE_START_PERCENT);
